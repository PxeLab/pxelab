...
const systemdServicePath = "/etc/systemd/system/pxego.service"

// launchdPlistPath is where launchd plists go.
const launchdPlistPath = "/Library/LaunchDaemons/com.pxego.plist"
...
Documentation=https://github.com/pxego/pxego
...
`, exe)
	...
	if err := exec.Command("systemctl", "enable", "pxego").Run(); err != nil {
...
func removeSystemd() error {
	exec.Command("systemctl", "disable", "pxego").Run()
...
func runSystemctl(action string) error {
	cmd := exec.Command("systemctl", action, "pxego")
	if out, err := cmd.CombinedOutput(); err != nil {
...
<dict>
	...
	<string>/var/log/pxego.log</string>
