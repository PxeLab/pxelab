...
PxeLab 是一个 Go 语言开发的一体化 PXE 服务器，完整复现并扩展 Tiny PXE Server 的全部功能。单二进制、跨平台（Windows/Linux/macOS），双模式运行（无桌面环境的命令行启动模式 + 浏览器管理界面）。
...
- `cmd/pxelab/main.go` — cobra 根命令、配置加载、信号处理
- **交付物**：`PxeLab --help` 可用，配置文件解析完成，SQLite 数据库创建
...
- macOS：launchd 兼容
- `PxeLab service install|remove|start|stop`
...
## PxeLab vs Tiny PXE Server 功能对照
