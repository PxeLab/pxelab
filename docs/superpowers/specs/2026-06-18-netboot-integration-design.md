# pxelab netboot.xyz 集成设计文档

## 概述

将 netboot.xyz 的完整操作系统引导目录集成到 PxeLab 中，作为 PxeLab 的内置组件。
...
│  │        │            │                  │              │ │
│  │        ▼            ▼                  ▼              │ │
│  │  ~/.pxelab/netboot/catalog/*.yaml                      │ │
│  │  ~/.pxelab/netboot/scripts/*.ipxe                     │ │
│  │  ~/.pxelab/boot/netboot/<distro>/<ver>/               │ │
│  └──────────────────────────────────────────────────────┘ │
└─────────────────────────────────────────────────────────┘
...
| B. PxeLab catalog YAML | `~/.pxelab/netboot/catalog/*.yaml` | PxeLab 内部目录，由 sync 生成 | 是（可删/改/增） |
| C. 预设 iPXE 脚本 | `~/.pxelab/netboot/scripts/*.ipxe` | TFTP fallback + 参考 | 是 |
...
```yaml
# ~/.pxelab/netboot/catalog/ubuntu.yaml
name: Ubuntu
enabled: true
...
DHCP 返回通用 iPXE 固件 (undionly.kpxe / ipxe.efi)
    │
    ▼
iPXE 启动 → HTTP 请求 http://<PxeLab>/boot/ipxe/script?mac=XX
    │
    ▼
PxeLab 生成 iPXE 脚本：
    │
...
```
# PxeLab 生成的 iPXE 脚本逻辑
:boot_<distro>_<version>
imgfree

# 优先级 1: 本地文件存在 → PxeLab HTTP 本地服务
# （用户在 ~/.pxelab/boot/netboot/ 放置了文件）
kernel http://<PxeLab>/boot/netboot/ubuntu/24.04/vmlinuz root=/dev/ram0 ...
initrd http://<PxeLab>/boot/netboot/ubuntu/24.04/initrd
boot
...
4. **引导文件管理**
   - 显示本地已有/缺失的文件
   - 给出下载命令指引（如 `wget <url> -O ~/.pxelab/boot/netboot/...`）
...
# 同步并重新生成预设 iPXE 脚本
pxelab netboot sync --generate-scripts

# 列出所有可用发行版
pxelab netboot list

# 查看某个发行版的详细信息
pxelab netboot info <distro>

# 检查本地引导文件状态
pxelab netboot check-files

# 生成 TFTP fallback 脚本
pxelab netboot generate-scripts
```

## 同步机制 (`pxelab netboot sync`)
...
5. 将 Ansible YAML 格式转换为 PxeLab catalog YAML 格式
6. 写入 `~/.pxelab/netboot/catalog/<distro>.yaml`（每个发行版一个文件）
7. 如果 `--generate-scripts`，生成预设 iPXE 脚本到 `netboot/scripts/`
8. 记录本次同步时间戳

## iPXE 脚本生成逻辑

PxeLab 的 `internal/boot/ipxe/templates.go` 会新增一个模板类型 `netboot-menu`。
...
:noble
kernel http://<PxeLab>/boot/netboot/ubuntu/24.04/vmlinuz ...
initrd http://<PxeLab>/boot/netboot/ubuntu/24.04/initrd
boot
```
...
PxeLab 的 `config.yaml` 新增 `netboot` 段：
...
netboot:
  ...
  paths:
    catalog: ~/.pxelab/netboot/catalog
    scripts: ~/.pxelab/netboot/scripts
...
| 4 | CLI 命令 (`pxelab netboot sync/list/info`) | 2-3 |
